...
def create_product(request):
    if request.method == 'GET':
        ...
        if product_form.is_valid():
            ...
            for i in range(3):
                image = product_form.cleaned_data.get("image_{}".formate(i + 1))
                if image:
                    product_image = ProductImage.objects.create(product=product, url=image)
...
def edit_product(request, product_id):
    ...
    if request.method == 'GET':
        ...
        if product_form.is_valid():
            ...
            for i in range(3):
                image_url = product_form.cleaned_data.get("image_{}".format(i + 1))
                if image_url:
...
def remove_from_cart(request):
    ...
    if reqeust.session.get('products_in_cart'):
