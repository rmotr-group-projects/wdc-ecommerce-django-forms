...
def products(request):
    ...
    if products.filter(featured=True).count()>=4:
...
def edit_product(request, product_id):
    ...
    if request.method == 'GET':
        ...
        if product_form.is_valid():
            ...
            current_images_urls = [pic.url for pic in current_images]
            new_images = []
            for i in range(3):
                input_url = product_form.cleaned_data['image_{}'.format(i+1)]
                if input_url:
                    new_images.append(input_url)
                    ProductImage.objects.create(product = product,url=input_url)
            for url in current_images_urls:
                if url not in new_images:
                    del_pic = product.productimage_set.filter(url=url)
                    del_pic.delete()
            # Redirect to 'products' view
            return redirect('products')
